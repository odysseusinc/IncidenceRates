...
FROM
(
  SELECT  2  as cohort_id, 0 as is_outcome UNION SELECT  5  as cohort_id, 0 as is_outcome  UNION  SELECT  3164  as cohort_id, 1 as is_outcome
) C
;
...
(
  ...
  join (
    select cohort_definition_id, subject_id, cohort_start_date, cohort_end_date, (cohort_start_date  + 0 *INTERVAL'1 day') as adjusted_start_date, (cohort_start_date  + 365 *INTERVAL'1 day') as adjusted_end_date
    FROM results.cohort
  ) t on t.cohort_definition_id = combos.target_id
  join public.observation_period op on t.subject_id = op.person_id and t.cohort_start_date between op.observation_period_start_date and op.observation_period_end_date
  left join (
...
DELETE FROM results.ir_analysis_result where analysis_id = 3178;

INSERT INTO results.ir_analysis_result (analysis_id, target_id, outcome_id, strata_mask, person_count, time_at_risk, cases)
select 3178 as analysis_id, T.target_id, T.outcome_id, E.strata_mask,
...
delete from results.ir_analysis_strata_stats where analysis_id = 3178;
insert into results.ir_analysis_strata_stats (analysis_id, target_id, outcome_id, strata_sequence, person_count, time_at_risk, cases)
...
WHERE irs.analysis_id = 3178
;

-- calculate distributions for TAR and TTO by strata

DELETE FROM results.ir_analysis_dist where analysis_id = 3178;
...
INSERT INTO results.ir_analysis_dist (analysis_id, dist_type, target_id, outcome_id, strata_sequence, total, avg_value, std_dev,min_value, p10_value, p25_value, median_value, p75_value, p90_value,max_value)
select 3178 as analysis_id, 1 as dist_type, combos.target_id, combos.outcome_id, 
...
INSERT INTO results.ir_analysis_dist (analysis_id, dist_type, target_id, outcome_id, strata_sequence, total, avg_value, std_dev,min_value, p10_value, p25_value, median_value, p75_value, p90_value,max_value)
select 3178 as analysis_id, 2 as dist_type, combos.target_id, combos.outcome_id, 
  case when d.strata_sequence = -1 then null else d.strata_sequence end as strata_sequence, 
  d.total, d.avg_value, d.stdev_value, d.min_value, d.p10_value, d.p25_value, d.median_value, d.p75_value, d.p90_value, d.max_value
FROM 
(
